...
typedef struct {
    ...
    bit [3:0] sp;                 // Stack pointer (0 to 7)
    bit full;
    bit empty;
} lifo_t;
...
module display (
    ...
    always_comb begin
        ...
        cathodes[7:1] = segment_pattern[6:0];
        if (digit_select == lifo_state.sp) begin
            cathodes[0] = 1'b0; // DP on
        end else begin
            cathodes[0] = 1'b1; // DP off
        end
    end
endmodule
